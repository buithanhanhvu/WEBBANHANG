...
## 10. Quản Lý Đơn Hàng, Theo Dõi Trạng Trái & Hủy Đơn Hàng (Order Tracking, Status Updates & Cancellation)

### ❓ Câu hỏi của Thầy/Cô có thể gặp:
*   *Quy trình theo dõi đơn hàng hoạt động thế nào?*
*   *Khách hàng có thể hủy đơn hàng khi nào? Logic xử lý tồn kho khi hủy đơn ra sao?*
*   *Làm thế nào để tránh tình trạng tranh chấp dữ liệu (Race Condition) khi Admin khôi phục lại một đơn hàng đã hủy về trạng thái bình thường?*

### 💡 Nguyên lý hoạt động & Thiết kế:
*   **Theo dõi đơn hàng (Order History & Tracking):** Khách hàng sau khi đặt hàng có thể theo dõi danh sách đơn hàng đã mua (`GET /api/orders`) và chi tiết từng sản phẩm trong một đơn hàng cụ thể (`GET /api/orders/{id}`).
*   **Quy trình cập nhật trạng thái đơn hàng (Admin):** Admin có thể chuyển đổi trạng thái đơn hàng qua các bước: `PENDING` (Chờ xử lý) -> `DELIVERED` (Đã giao hàng) hoặc `CANCELLED` (Đã hủy).
*   **Hủy đơn hàng và Hoàn lại tồn kho (Stock Rollback):**
    *   **Khách hàng:** Chỉ được quyền hủy đơn hàng của mình khi trạng thái đơn hàng vẫn đang ở mức **`PENDING`** (chờ xác nhận). Nếu đơn hàng đã sang trạng thái khác, hệ thống sẽ từ chối.
    *   Khi hủy đơn thành công (cho cả Khách hàng tự hủy hoặc Admin chuyển trạng thái thành `CANCELLED`), hệ thống sẽ duyệt qua danh sách sản phẩm trong đơn (`order_items`) và **hoàn trả lại số lượng** tương ứng vào tồn kho (`stock`) của từng sản phẩm trong bảng `products`.
*   **Bảo vệ tranh chấp dữ liệu (Pessimistic Lock):**
    *   Nếu một đơn hàng đã bị hủy (đã được hoàn trả tồn kho), sau đó Admin muốn "Khôi phục" hoặc chuyển nó về trạng thái bình thường (`PENDING`/`DELIVERED`...), hệ thống bắt buộc phải trừ lại số lượng tồn kho tương ứng.
    *   Để tránh race condition (nhiều tiến trình cùng cập nhật số lượng kho cùng lúc dẫn đến số lượng âm hoặc sai lệch), Backend sử dụng **Pessimistic Write Lock (`LockModeType.PESSIMISTIC_WRITE`)** tại tầng JPA Database để khóa dòng dữ liệu của sản phẩm đó lại: `entityManager.find(Product.class, productId, LockModeType.PESSIMISTIC_WRITE)`.
    *   Nếu số lượng tồn kho thực tế hiện tại không đủ để trừ, hệ thống sẽ quăng ngoại lệ `BadRequestException` và chặn đứng hành động khôi phục này.

### 🛠️ Các file Code & Database liên quan:
*   **Backend:**
    *   Thực thể & DB: [Order.java](file:///e:/WEBBANHANG/src/main/java/com/example/webbanhang/domain/Order.java) (bảng `orders`), [OrderItem.java](file:///e:/WEBBANHANG/src/main/java/com/example/webbanhang/domain/OrderItem.java) (bảng `order_items`).
    *   Xử lý logic: `ShopService.cancelOrder()` (hoàn tồn kho khi user hủy), `ShopService.updateOrderStatus()` (xử lý lock và tính toán tồn kho khi Admin đổi trạng thái).
*   **Frontend:**
    *   Khách hàng theo dõi và hủy đơn: [MyOrders.tsx](file:///e:/WEBBANHANG/frontend/src/pages/MyOrders.tsx).
    *   Admin quản lý và cập nhật đơn hàng: [AdminDashboard.tsx](file:///e:/WEBBANHANG/frontend/src/pages/AdminDashboard.tsx) (Tab Orders).

---

## 11. Quản Trị Admin Dashboard & Báo Cáo Doanh Thu (Admin Analytics & Alerts)

### ❓ Câu hỏi của Thầy/Cô có thể gặp:
*   *Làm sao để tính toán doanh thu tổng, thống kê cơ cấu doanh thu theo danh mục và vẽ biểu đồ?*
*   *Hệ thống đưa ra cảnh báo hàng sắp hết kho (Low Stock Alert) như thế nào?*

### 💡 Nguyên lý hoạt động & Thiết kế:
*   **Thống kê tổng hợp (KPI Cards):** Backend truy vấn trực tiếp SQL bằng Native Query kết hợp các hàm tổng hợp như `COUNT` và `SUM` có lọc theo khoảng thời gian (`created_at >= ?`) để trả về các thông số:
    *   Tổng số sản phẩm (`COUNT(*)` từ bảng `products`).
    *   Tổng số đơn giao thành công (`COUNT(*)` từ bảng `orders` có `status = 'DELIVERED'`).
    *   Tổng số khách hàng (`COUNT(*)` từ bảng `users` có `role = 'CUSTOMER'`).
    *   Tổng doanh thu thực tế (`SUM(total_amount)` từ bảng `orders` có `status = 'DELIVERED'`).
*   **Cơ cấu doanh thu theo danh mục (Category Revenue Distribution):**
    Sử dụng truy vấn SQL kết nối nhiều bảng (Joins) giữa `order_items`, `products`, `categories` và `orders` để gom nhóm và cộng tổng doanh thu thực tế của các đơn hàng thành công theo từng danh mục:
    `SELECT c.name, SUM(oi.price * oi.quantity) ... GROUP BY c.name`. Dữ liệu này được gửi về dạng JSON và thư viện biểu đồ ở Frontend (ví dụ: Chart.js hoặc Recharts) sẽ tự động vẽ thành biểu đồ tròn trực quan.
*   **Cảnh báo tồn kho thấp (Low Stock Alert):**
    Hệ thống liên tục kiểm tra và tìm các sản phẩm có số lượng tồn kho nhỏ hơn hoặc bằng 10 (`stock <= 10`) để hiển thị cảnh báo đỏ trên Dashboard cho Admin kịp thời nhập thêm hàng:
    `SELECT id, name, stock FROM products WHERE stock <= 10 ORDER BY stock ASC`.

### 🛠️ Các file Code & Database liên quan:
*   **Backend:**
    *   Xử lý logic và Query dữ liệu: `ShopService.dashboard(String period)`.
    *   Controller: [AdminController.java](file:///e:/WEBBANHANG/src/main/java/com/example/webbanhang/controller/admin/AdminController.java) (endpoint `/api/admin/dashboard`).
*   **Frontend:**
    *   Trang Dashboard hiển thị biểu đồ và cảnh báo: [AdminDashboard.tsx](file:///e:/WEBBANHANG/frontend/src/pages/AdminDashboard.tsx) (Tab Tổng quan).

---

## 12. Cơ Chế Tải Lên Tập Tin & Hình Ảnh Sản Phẩm (File Uploading Mechanism)

### ❓ Câu hỏi của Thầy/Cô có thể gặp:
*   *Hình ảnh sản phẩm được tải lên và lưu trữ ở đâu trên hệ thống?*
*   *Làm sao để đảm bảo tên file ảnh tải lên không bị trùng lặp hoặc ghi đè dữ liệu cũ?*
*   *Giải pháp nào giúp hiển thị ảnh vừa tải lên ngay trên giao diện web mà không cần khởi động lại máy chủ Spring Boot?*

### 💡 Nguyên lý hoạt động & Thiết kế:
*   **Multipart HTTP Request:** Client gửi ảnh dưới dạng tập tin đa phần (`MultipartFile`) thông qua phương thức `POST` gửi đến API `/api/upload`.
*   **Xác thực và Kiểm tra định dạng:**
    *   Chỉ cho phép người dùng đã đăng nhập sử dụng API này.
    *   Kiểm tra định dạng file thông qua `file.getContentType()` bắt buộc phải bắt đầu bằng `image/` (chặn các file độc hại khác như `.exe`, `.sh`, `.php`...).
*   **Tránh trùng tên bằng UUID:** Hệ thống tạo ra một tên file ngẫu nhiên và duy nhất bằng cách sinh chuỗi **UUID (Universally Unique Identifier)** kết hợp với phần mở rộng của file ảnh gốc. Ví dụ: `a1b2c3d4-e5f6-...png`. Việc này đảm bảo không bao giờ xảy ra tình trạng trùng tên đè file.
*   **Cơ chế lưu trữ kép (Dual Directory Storage):**
    Khi lưu file cục bộ, nếu chỉ lưu vào thư mục source code thì server sẽ không nhận diện được ngay do chưa build. Ngược lại, nếu chỉ lưu vào thư mục build (`target/`) thì khi Clean & Rebuild dự án, toàn bộ ảnh sẽ bị xóa mất. Vì vậy, hệ thống sử dụng cơ chế lưu trữ kép:
    1.  Lưu vào thư mục tài nguyên gốc: `src/main/resources/static/uploads` (lưu trữ lâu dài để không bị mất khi đóng gói dự án).
    2.  Lưu vào thư mục thực thi của server: `target/classes/static/uploads` (giúp Spring Boot nhận diện ảnh và cấp URL truy cập được ngay lập tức cho client mà không cần restart server).

### 🛠️ Các file Code & Database liên quan:
*   **Backend:**
    *   Controller xử lý upload: [UploadController.java](file:///e:/WEBBANHANG/src/main/java/com/example/webbanhang/controller/UploadController.java).
    *   Cấu hình đường dẫn thư mục lưu trữ: cấu hình trong file [application.properties](file:///e:/WEBBANHANG/src/main/resources/application.properties) (`app.upload.dir=uploads`).
*   **Frontend:**
    *   Form UI upload ảnh sản phẩm: Nằm trong phần CRUD sản phẩm của [AdminDashboard.tsx](file:///e:/WEBBANHANG/frontend/src/pages/AdminDashboard.tsx).
